...
class top_camera(LifecycleNode):
 ...
 #lifecycle is used to activate simple commander api 27/9/2024    
    ...
    def map_callback(self):
        map = OccupancyGrid()
        #gazebo localization and mapping
    ...
    def colour_localization_mapping(self):
        ...
        if object_counter(mask_blue) != 0 and object_counter(mask_green) != 0:
            ...
            maze = mapping(self.current_frame,blue_rowcol,85)
...
def mapping(image:np.ndarray,point: list[int], size: int)->np.ndarray:
    ...
    start_x = point[0] - size // 2
    end_x = point[0] + size // 2
    start_y = point[1] - size // 2
    end_y = point[1] + size // 2
    
    # Handle the case for even n, where the square size isn't symmetric
    if size % 2 == 0:
        end_x -= 1
        end_y -= 1
    
    # Set the values inside the square to 0
...
def erase_start_goal(maze: np, min_row,min_col):
    size = 53   #pixels
    max_row = min(maze.shape[0],min_row + size)
    max_col = min(maze.shape[0],min_row + size)
    return 0
